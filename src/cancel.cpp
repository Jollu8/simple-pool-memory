...
#error Lock-free atomic required for signaml safty
#endif


// ПРИМЕЧАНИЕ: Мы заботимся в основном о том, чтобы общий случай (не отмененный) выполнялся быстро.
    ...
    Status StopToken::Poll() const {
        ...
        if (impl_->requested_.load()) return Status::OK();
